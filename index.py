from flask import Flask, request, jsonify, Response
import json, os, hashlib, uuid
from datetime import datetime, timedelta

app = Flask(__name__)

DATA_FILE = '/tmp/tp_db.json'
SECRET    = os.environ.get('APP_SECRET', 'tp_secret_2024_xyz')


# ─────────────────────────────────────────────
# DB helpers
# ─────────────────────────────────────────────
def read_db():
    try:
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, 'r') as f:
                data = json.load(f)
                if 'users' not in data:
                    data['users'] = {}
                return data
    except Exception as e:
        print(f"read_db error: {e}")
    return {'users': {}}


def write_db(db):
    try:
        with open(DATA_FILE, 'w') as f:
            json.dump(db, f)
    except Exception as e:
        print(f"write_db error: {e}")


# ─────────────────────────────────────────────
# Token helpers  (simple, deterministic, no expiry)
# ─────────────────────────────────────────────
def make_token(uid):
    """Token = sha256(uid + SECRET) + '.' + uid  — stable, no timestamp."""
    h = hashlib.sha256(f"{uid}{SECRET}".encode()).hexdigest()
    return f"{h}.{uid}"


def uid_from_token(token):
    """Return uid if token is valid, else None."""
    if not token or '.' not in token:
        return None
    parts = token.rsplit('.', 1)
    if len(parts) != 2:
        return None
    uid = parts[1]
    expected = make_token(uid)
    if token == expected:
        return uid
    return None


def get_uid_from_request():
    auth = request.headers.get('Authorization', '')
    if auth.startswith('Bearer '):
        token = auth[7:].strip()
    else:
        token = request.args.get('token', '').strip()
    return uid_from_token(token)


# ─────────────────────────────────────────────
# User data helpers
# ─────────────────────────────────────────────
DEFAULT_CATS = [
    {"id": 1,  "name": "General",     "color": "#6366f1"},
    {"id": 2,  "name": "Work",        "color": "#0ea5e9"},
    {"id": 3,  "name": "Personal",    "color": "#f59e0b"},
    {"id": 4,  "name": "Meetings",    "color": "#8b5cf6"},
    {"id": 5,  "name": "Development", "color": "#10b981"},
    {"id": 6,  "name": "Design",      "color": "#ec4899"},
    {"id": 7,  "name": "Marketing",   "color": "#f97316"},
    {"id": 8,  "name": "Finance",     "color": "#14b8a6"},
    {"id": 9,  "name": "HR",          "color": "#6b7280"},
    {"id": 10, "name": "Urgent",      "color": "#ef4444"},
]

DEFAULT_SETTINGS = {
    "default_reminder_low": 60, "default_reminder_medium": 30,
    "default_reminder_high": 15, "sound_enabled": 1, "popup_enabled": 1,
    "browser_notif_enabled": 1, "popup_duration_low": 5,
    "popup_duration_medium": 8, "popup_duration_high": 12,
    "auto_snooze_mins": 10, "check_interval_secs": 30,
}


def blank_user(name, email, pw_hash, role='Member'):
    return {
        "name": name, "email": email,
        "password": pw_hash, "role": role,
        "created_at": _now(),
        "tasks": [],
        "categories": json.loads(json.dumps(DEFAULT_CATS)),
        "notifications": [],
        "settings": json.loads(json.dumps(DEFAULT_SETTINGS)),
        "ids": {"tasks": 1, "categories": 11, "notifications": 1},
    }


def hash_pw(pw):
    return hashlib.sha256(f"{pw}{SECRET}".encode()).hexdigest()


def _now():
    return datetime.now().isoformat()


def _nxt(ud, col):
    v = ud['ids'].get(col, 1)
    ud['ids'][col] = v + 1
    return v


# ─────────────────────────────────────────────
# Notification engine
# ─────────────────────────────────────────────
def run_notifications(ud):
    now = datetime.now()
    today = now.strftime('%Y-%m-%d')
    tomorrow = (now + timedelta(days=1)).strftime('%Y-%m-%d')

    # build set of existing keys
    ex = set()
    for n in ud.get('notifications', []):
        if not n.get('is_dismissed'):
            ex.add(f"{n.get('task_id')}_{n.get('type')}")

    for t in ud.get('tasks', []):
        if t.get('status') == 'completed' or not t.get('due_date'):
            continue
        tid = t['id']
        pri = t.get('priority', 'medium')
        em  = '🔴' if pri == 'high' else '🟡' if pri == 'medium' else '🟢'
        dd  = t['due_date']

        if dd < today:
            k = f"{tid}_overdue"
            if k not in ex:
                days = max(1, (now - datetime.strptime(dd, '%Y-%m-%d')).days)
                _push_notif(ud, tid, f"{em} OVERDUE ({days}d): '{t['title']}'",
                            'overdue', pri, t['title'])

        elif dd == today:
            k = f"{tid}_due_today"
            if k not in ex:
                ti = f" at {t['due_time']}" if t.get('due_time') else ''
                _push_notif(ud, tid,
                            f"{em} Due Today{ti}: '{t['title']}'",
                            'due_today', pri, t['title'])

        if t.get('due_time') and not t.get('reminder_sent'):
            try:
                due = datetime.strptime(f"{dd} {t['due_time']}", '%Y-%m-%d %H:%M')
                rm  = t.get('reminder_mins', 30) or 30
                ra  = due - timedelta(minutes=rm)
                if ra <= now < due:
                    k = f"{tid}_reminder"
                    if k not in ex:
                        ml = max(0, int((due - now).total_seconds() / 60))
                        _push_notif(ud, tid,
                                    f"{em} REMINDER: '{t['title']}' in {ml} min!",
                                    'reminder', pri, t['title'])
                        t['reminder_sent'] = 1
            except Exception:
                pass

        if pri == 'high' and dd == tomorrow:
            k = f"{tid}_upcoming_high"
            if k not in ex:
                _push_notif(ud, tid,
                            f"🔴 HIGH PRIORITY tomorrow: '{t['title']}'",
                            'upcoming_high', 'high', t['title'])

    ud['notifications'] = ud['notifications'][:80]


def _push_notif(ud, tid, msg, ntype, pri, title):
    ud['notifications'].insert(0, {
        'id': _nxt(ud, 'notifications'), 'task_id': tid,
        'message': msg, 'type': ntype, 'priority': pri,
        'is_read': 0, 'is_dismissed': 0, 'created_at': _now(),
        'task_title': title, 'task_priority': pri,
    })


# ─────────────────────────────────────────────
# CORS
# ─────────────────────────────────────────────
@app.after_request
def add_cors(r):
    r.headers['Access-Control-Allow-Origin']  = '*'
    r.headers['Access-Control-Allow-Headers'] = 'Content-Type,Authorization'
    r.headers['Access-Control-Allow-Methods'] = 'GET,POST,PUT,DELETE,OPTIONS'
    return r


@app.before_request
def handle_options():
    if request.method == 'OPTIONS':
        r = jsonify({})
        r.headers['Access-Control-Allow-Origin']  = '*'
        r.headers['Access-Control-Allow-Headers'] = 'Content-Type,Authorization'
        r.headers['Access-Control-Allow-Methods'] = 'GET,POST,PUT,DELETE,OPTIONS'
        return r, 200


# ─────────────────────────────────────────────
# Serve HTML
# ─────────────────────────────────────────────
@app.route('/')
def serve():
    p = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                     'static', 'app.html')
    try:
        with open(p, 'r', encoding='utf-8') as f:
            return Response(f.read(), content_type='text/html; charset=utf-8')
    except Exception as e:
        return Response(f'<h1>Cannot load app.html: {e}</h1>',
                        content_type='text/html')


# ─────────────────────────────────────────────
# AUTH
# ─────────────────────────────────────────────
@app.route('/api/auth/signup', methods=['POST'])
def signup():
    b     = request.json or {}
    name  = (b.get('name')  or '').strip()
    email = (b.get('email') or '').strip().lower()
    pw    = (b.get('password') or '')
    role  = (b.get('role')  or 'Member').strip() or 'Member'

    if not name:
        return jsonify({'error': 'Name is required'}), 400
    if not email:
        return jsonify({'error': 'Email is required'}), 400
    if not pw:
        return jsonify({'error': 'Password is required'}), 400
    if len(pw) < 4:
        return jsonify({'error': 'Password must be at least 4 characters'}), 400

    db = read_db()
    for u in db['users'].values():
        if u.get('email', '').lower() == email:
            return jsonify({'error': 'Email already registered'}), 400

    uid = str(uuid.uuid4())[:12]
    db['users'][uid] = blank_user(name, email, hash_pw(pw), role)
    write_db(db)

    token = make_token(uid)
    return jsonify({
        'token': token,
        'user':  {'id': uid, 'name': name, 'email': email, 'role': role},
    })


@app.route('/api/auth/login', methods=['POST'])
def login():
    b     = request.json or {}
    email = (b.get('email')    or '').strip().lower()
    pw    = (b.get('password') or '')

    if not email or not pw:
        return jsonify({'error': 'Email and password required'}), 400

    db = read_db()
    for uid, u in db['users'].items():
        if u.get('email', '').lower() == email and u.get('password') == hash_pw(pw):
            token = make_token(uid)
            return jsonify({
                'token': token,
                'user':  {'id': uid, 'name': u['name'],
                          'email': u['email'], 'role': u.get('role', 'Member')},
            })

    return jsonify({'error': 'Invalid email or password'}), 401


@app.route('/api/auth/guest', methods=['POST'])
def guest():
    db  = read_db()
    uid = 'g_' + str(uuid.uuid4())[:8]
    em  = f"{uid}@guest.taskpro"
    db['users'][uid] = blank_user('Guest User', em, '', 'Guest')
    write_db(db)
    token = make_token(uid)
    return jsonify({
        'token': token,
        'user':  {'id': uid, 'name': 'Guest User', 'email': em, 'role': 'Guest'},
    })


@app.route('/api/auth/me')
def auth_me():
    uid = get_uid_from_request()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    db = read_db()
    u  = db['users'].get(uid)
    if not u:
        return jsonify({'error': 'User not found'}), 404
    return jsonify({'user': {'id': uid, 'name': u['name'],
                             'email': u['email'], 'role': u.get('role', 'Member')}})


@app.route('/api/auth/profile', methods=['PUT'])
def update_profile():
    uid = get_uid_from_request()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    db = read_db()
    u  = db['users'].get(uid)
    if not u:
        return jsonify({'error': 'User not found'}), 404
    b = request.json or {}
    if (b.get('name') or '').strip():
        u['name'] = b['name'].strip()
    if b.get('role') is not None:
        u['role'] = (b['role'] or '').strip()
    if (b.get('email') or '').strip():
        u['email'] = b['email'].strip().lower()
    write_db(db)
    return jsonify({'user': {'id': uid, 'name': u['name'],
                             'email': u['email'], 'role': u.get('role', 'Member')}})


# ─────────────────────────────────────────────
# Internal auth helper
# ─────────────────────────────────────────────
def _auth():
    """Returns (uid, ud, db) or raises 401 response."""
    uid = get_uid_from_request()
    if not uid:
        return None, None, None
    db = read_db()
    ud = db['users'].get(uid)
    if not ud:
        return None, None, None
    return uid, ud, db


def _save(db, uid, ud):
    db['users'][uid] = ud
    write_db(db)


# ─────────────────────────────────────────────
# Settings
# ─────────────────────────────────────────────
@app.route('/api/settings', methods=['GET', 'PUT'])
def settings():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    if request.method == 'PUT':
        for k, v in (request.json or {}).items():
            if k in ud['settings']:
                ud['settings'][k] = v
        _save(db, uid, ud)
    return jsonify(ud['settings'])


# ─────────────────────────────────────────────
# Tasks
# ─────────────────────────────────────────────
@app.route('/api/tasks', methods=['GET', 'POST'])
def tasks_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401

    if request.method == 'POST':
        b   = request.json or {}
        pri = b.get('priority', 'medium')
        dr  = {
            'low':    ud['settings'].get('default_reminder_low',    60),
            'medium': ud['settings'].get('default_reminder_medium', 30),
            'high':   ud['settings'].get('default_reminder_high',   15),
        }
        ts   = _now()
        task = {
            'id':            _nxt(ud, 'tasks'),
            'title':         (b.get('title') or 'Untitled').strip(),
            'description':   b.get('description', ''),
            'category':      b.get('category', 'General'),
            'priority':      pri,
            'status':        b.get('status', 'pending'),
            'due_date':      b.get('due_date') or None,
            'due_time':      b.get('due_time') or None,
            'reminder_mins': b.get('reminder_mins', dr.get(pri, 30)),
            'assigned_to':   b.get('assigned_to', ''),
            'project':       b.get('project', ''),
            'tags':          json.dumps(b.get('tags', [])),
            'notes':         b.get('notes', ''),
            'progress':      b.get('progress', 0),
            'is_pinned':     1 if b.get('is_pinned') else 0,
            'created_at':    ts,
            'updated_at':    ts,
            'completed_at':  None,
            'reminder_sent': 0,
            'snooze_until':  None,
        }
        ud['tasks'].insert(0, task)
        run_notifications(ud)
        _save(db, uid, ud)
        return jsonify(task), 201

    # GET
    tl = list(ud['tasks'])
    for f in ['status', 'priority', 'category']:
        v = request.args.get(f, '')
        if v:
            tl = [t for t in tl if t.get(f) == v]
    q = (request.args.get('search', '') or '').lower()
    if q:
        tl = [t for t in tl
              if q in (t.get('title', '') or '').lower()
              or q in (t.get('description', '') or '').lower()]
    tl.sort(key=lambda t: t.get('is_pinned', 0), reverse=True)
    return jsonify(tl)


@app.route('/api/tasks/<int:tid>', methods=['GET', 'PUT', 'DELETE'])
def task_route(tid):
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401

    task = next((t for t in ud['tasks'] if t['id'] == tid), None)
    if not task:
        return jsonify({'error': 'Task not found'}), 404

    if request.method == 'GET':
        return jsonify(task)

    if request.method == 'DELETE':
        ud['tasks']         = [t for t in ud['tasks']         if t['id'] != tid]
        ud['notifications'] = [n for n in ud['notifications'] if n.get('task_id') != tid]
        _save(db, uid, ud)
        return jsonify({'ok': True})

    # PUT
    b      = request.json or {}
    old_st = task['status']
    for k in ['title', 'description', 'category', 'priority', 'status',
              'due_date', 'due_time', 'reminder_mins', 'assigned_to',
              'project', 'notes', 'progress', 'is_pinned', 'snooze_until']:
        if k in b:
            task[k] = b[k]
    if 'tags' in b:
        task['tags'] = json.dumps(b['tags']) if isinstance(b['tags'], list) else b['tags']
    task['updated_at'] = _now()
    new_st = task['status']
    if new_st == 'completed' and old_st != 'completed':
        task['completed_at'] = _now()
    elif new_st != 'completed':
        task['completed_at'] = None
    if 'due_date' in b or 'due_time' in b:
        task['reminder_sent'] = 0
    run_notifications(ud)
    _save(db, uid, ud)
    return jsonify(task)


@app.route('/api/tasks/<int:tid>/snooze', methods=['PUT'])
def snooze_task(tid):
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    mins = (request.json or {}).get('minutes', 10)
    su   = (datetime.now() + timedelta(minutes=mins)).isoformat()
    for t in ud['tasks']:
        if t['id'] == tid:
            t['snooze_until']  = su
            t['reminder_sent'] = 0
            break
    ud['notifications'] = [n for n in ud['notifications']
                           if not (n.get('task_id') == tid and n.get('type') == 'reminder')]
    _save(db, uid, ud)
    return jsonify({'ok': True})


@app.route('/api/tasks/<int:tid>/pin', methods=['PUT'])
def pin_task(tid):
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    for t in ud['tasks']:
        if t['id'] == tid:
            t['is_pinned'] = 0 if t.get('is_pinned') else 1
            _save(db, uid, ud)
            return jsonify({'is_pinned': t['is_pinned']})
    return jsonify({'error': 'Not found'}), 404


# ─────────────────────────────────────────────
# Categories
# ─────────────────────────────────────────────
@app.route('/api/categories', methods=['GET', 'POST'])
def categories_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    if request.method == 'POST':
        b    = request.json or {}
        name = (b.get('name') or '').strip()
        if not name:
            return jsonify({'error': 'Name required'}), 400
        if any(c['name'] == name for c in ud['categories']):
            return jsonify({'error': 'Category already exists'}), 400
        cat = {'id': _nxt(ud, 'categories'), 'name': name,
               'color': b.get('color', '#6366f1')}
        ud['categories'].append(cat)
        _save(db, uid, ud)
        return jsonify(cat), 201
    return jsonify(ud['categories'])


@app.route('/api/categories/<int:cid>', methods=['DELETE'])
def del_category(cid):
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    ud['categories'] = [c for c in ud['categories'] if c['id'] != cid]
    _save(db, uid, ud)
    return jsonify({'ok': True})


# ─────────────────────────────────────────────
# Notifications
# ─────────────────────────────────────────────
@app.route('/api/notifications')
def notifs_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    run_notifications(ud)
    _save(db, uid, ud)
    ns = [n for n in ud['notifications'] if not n.get('is_dismissed')]
    po = {'high': 0, 'medium': 1, 'low': 2}
    ns.sort(key=lambda n: po.get(n.get('priority', 'medium'), 1))
    return jsonify(ns)


@app.route('/api/notifications/new')
def notifs_new():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    run_notifications(ud)
    _save(db, uid, ud)
    ns = [n for n in ud['notifications']
          if not n.get('is_read') and not n.get('is_dismissed')
          and n.get('type') != 'created']
    po = {'high': 0, 'medium': 1, 'low': 2}
    ns.sort(key=lambda n: po.get(n.get('priority', 'medium'), 1))
    return jsonify(ns)


@app.route('/api/notifications/<int:nid>/read', methods=['PUT'])
def read_notif(nid):
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    for n in ud['notifications']:
        if n['id'] == nid:
            n['is_read'] = 1
            break
    _save(db, uid, ud)
    return jsonify({'ok': True})


@app.route('/api/notifications/read-all', methods=['PUT'])
def read_all():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    for n in ud['notifications']:
        n['is_read'] = 1
    _save(db, uid, ud)
    return jsonify({'ok': True})


@app.route('/api/notifications/clear-read', methods=['DELETE'])
def clear_read():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    ud['notifications'] = [n for n in ud['notifications'] if not n.get('is_read')]
    _save(db, uid, ud)
    return jsonify({'ok': True})


# ─────────────────────────────────────────────
# Statistics
# ─────────────────────────────────────────────
@app.route('/api/statistics')
def stats_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    tl    = ud.get('tasks', [])
    today = datetime.now().strftime('%Y-%m-%d')
    total = len(tl)
    comp  = sum(1 for t in tl if t.get('status') == 'completed')
    pend  = sum(1 for t in tl if t.get('status') == 'pending')
    prog  = sum(1 for t in tl if t.get('status') == 'in_progress')
    ov    = sum(1 for t in tl if t.get('status') != 'completed'
                and t.get('due_date') and t['due_date'] < today)
    dt    = sum(1 for t in tl if t.get('status') != 'completed'
                and t.get('due_date') == today)
    hp    = sum(1 for t in tl if t.get('priority') == 'high'
                and t.get('status') != 'completed')
    return jsonify({
        'total': total, 'completed': comp, 'pending': pend,
        'in_progress': prog, 'overdue': ov, 'due_today': dt,
        'high_priority': hp,
        'completion_rate': round(comp / total * 100, 1) if total else 0,
    })


# ─────────────────────────────────────────────
# Export / Import
# ─────────────────────────────────────────────
@app.route('/api/export')
def export_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    return jsonify({'exported_at': _now(),
                    'tasks': ud.get('tasks', []),
                    'categories': ud.get('categories', [])})


@app.route('/api/import', methods=['POST'])
def import_route():
    uid, ud, db = _auth()
    if not uid:
        return jsonify({'error': 'Not authenticated'}), 401
    inc   = (request.json or {}).get('tasks', [])
    ts    = _now()
    count = 0
    for t in inc:
        ud['tasks'].insert(0, {
            'id':            _nxt(ud, 'tasks'),
            'title':         t.get('title', 'Imported'),
            'description':   t.get('description', ''),
            'category':      t.get('category', 'General'),
            'priority':      t.get('priority', 'medium'),
            'status':        t.get('status', 'pending'),
            'due_date':      t.get('due_date'),
            'due_time':      t.get('due_time'),
            'reminder_mins': t.get('reminder_mins', 30),
            'assigned_to':   t.get('assigned_to', ''),
            'project':       t.get('project', ''),
            'tags':          json.dumps(t.get('tags', [])),
            'notes':         t.get('notes', ''),
            'created_at': ts, 'updated_at': ts, 'completed_at': None,
            'reminder_sent': 0, 'is_pinned': 0, 'progress': 0,
            'snooze_until': None,
        })
        count += 1
    _save(db, uid, ud)
    return jsonify({'imported': count})


@app.route('/api/health')
def health():
    return jsonify({'status': 'ok', 'time': _now()})