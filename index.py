...
def sb_post(table, data):
    try:
        ...
        print(f"SB POST {table} {r.status_code}: {r.text[:200]}")
        return None
    except Exception as e:
    ...
    try:
        ...
        if r.status_code in (200, 204):
            try:
                ...
                return data
        print(f"SB PATCH {table} {r.status_code}: {r.text[:200]}")
        return None
    except Exception as e:
...
def gen_notifs_local(data):
    ...
    tasks = [t for t in data.get('tasks', []) if t.get('status') != 'completed' and t.get('due_date')]
    for t in tasks:
        tid = t['id']
        ...
        if dd < td:
            ...
            if k not in ex:
                ...
                notif = {'id': nid(data, 'notifications'), 'task_id': tid,
...
def gen_notifs_supabase():
    try:
        ...
        td = n.strftime('%Y-%m-%d')
        tm = (n + timedelta(days=1)).strftime('%Y-%m-%d')
        existing = sb_get('notifications', {'is_dismissed': 'eq.0', 'select': 'task_id,type'}) or []
        ex = set(f"{x.get('task_id')}_{x.get('type')}" for x in existing)
        ...
        for t in tasks:
            ...
            if dd < td:
                ...
                if k not in ex:
                    days = (n - datetime.strptime(dd, '%Y-%m-%d')).days
                    sb_post('notifications', {'task_id': tid, 'message': f"{u} OVERDUE ({days}d): '{t['title']}'",
                                              'type': 'overdue', 'priority': pri, 'is_read': 0, 'is_dismissed': 0,
                                              'created_at': now(), 'task_title': t['title'], 'task_priority': pri})
            elif dd == td:
                k = f"{tid}_due_today"
                if k not in ex:
                    ti = f" at {t['due_time']}" if t.get('due_time') else ''
                    sb_post('notifications', {'task_id': tid, 'message': f"{u} Due Today{ti}: '{t['title']}'",
                                              'type': 'due_today', 'priority': pri, 'is_read': 0, 'is_dismissed': 0,
                                              'created_at': now(), 'task_title': t['title'], 'task_priority': pri})
            if t.get('due_time') and not t.get('reminder_sent'):
                try:
                    ...
                    if n >= ra and n < due:
                        ...
                        if k not in ex:
                            ml = max(0, int((due - n).total_seconds() / 60))
                            sb_post('notifications', {'task_id': tid,
                                                      'message': f"{u} REMINDER: '{t['title']}' in {ml} min!",
                                                      'type': 'reminder', 'priority': pri, 'is_read': 0, 'is_dismissed': 0,
                                                      'created_at': now(), 'task_title': t['title'], 'task_priority': pri})
...
def root():
    html_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static', 'app.html')
    try:
        with open(html_path, 'r', encoding='utf-8') as f:
            return Response(f.read(), content_type='text/html; charset=utf-8')
    except Exception as e:
        return Response(f'<h1>Error loading app.html: {e}</h1>', content_type='text/html')
...
def api_tasks():
    if USE_SUPABASE:
        if request.method == 'POST':
            ...
            dr = {'low': se.get('default_reminder_low', 60),
                  'medium': se.get('default_reminder_medium', 30),
                  'high': se.get('default_reminder_high', 15)}
            ts = now()
            task_data = {
                'title': b.get('title', 'Untitled'), 'description': b.get('description', ''),
                'category': b.get('category', 'General'), 'priority': pri,
        ...
        if se:
            sq = se.lower()
            tasks = [t for t in tasks if sq in (t.get('title', '') or '').lower()
        ...
        if request.method == 'POST':
            b = request.json or {}
            pri = b.get('priority', 'medium')
            dr = {'low': data['settings'].get('default_reminder_low', 60),
        ...
        if se: tasks = [t for t in tasks if se in (t.get('title', '') or '').lower()
...
def api_notifications():
    if USE_SUPABASE:
        gen_notifs_supabase()
        ns = sb_get('notifications', {'is_dismissed': 'eq.0', 'order': 'created_at.desc', 'limit': '80'}) or []
    else:
        data = ld()
        gen_notifs_local(data)
        sv(data)
...
def api_notifications_new():
    if USE_SUPABASE:
        gen_notifs_supabase()
        ns = sb_get('notifications', {'is_read': 'eq.0', 'is_dismissed': 'eq.0', 'order': 'created_at.desc'}) or []
    else:
        data = ld()
        gen_notifs_local(data)
        sv(data)
        ns = [n for n in data.get('notifications', []) if not n.get('is_read') and not n.get('is_dismissed')]
    po = {'high': 0, 'medium': 1, 'low': 2}
    ns.sort(key=lambda n: po.get(n.get('priority', 'medium'), 1))
    return jsonify(ns)
    ...
    if USE_SUPABASE:
        try:
            req.patch(f'{SB_URL}/rest/v1/notifications',
                      headers=sb_headers(), params={'is_dismissed': 'eq.0'},
    ...
    if USE_SUPABASE:
        try:
            req.delete(f'{SB_URL}/rest/v1/notifications',
                       headers=sb_headers(), params={'is_read': 'eq.1'}, timeout=8)
        except Exception as e:
...
def api_statistics():
    ...
    ov    = sum(1 for t in tasks if t.get('status') != 'completed' and t.get('due_date', '') < td and t.get('due_date'))
    dt    = sum(1 for t in tasks if t.get('status') != 'completed' and t.get('due_date') == td)
    hp    = sum(1 for t in tasks if t.get('priority') == 'high' and t.get('status') != 'completed')
    return jsonify({
        'total': total, 'completed': comp, 'pending': pend,
        'in_progress': prog, 'overdue': ov, 'due_today': dt, 'high_priority': hp,
...
def api_import():
    ...
    if USE_SUPABASE:
        for t in inc:
            sb_post('tasks', {
                'title': t.get('title', 'Imported'), 'description': t.get('description', ''),
                'category': t.get('category', 'General'), 'priority': t.get('priority', 'medium'),
                'status': t.get('status', 'pending'), 'due_date': t.get('due_date'),
                'due_time': t.get('due_time'), 'reminder_mins': t.get('reminder_mins', 30),
                'assigned_to': t.get('assigned_to', ''), 'project': t.get('project', ''),
                'tags': json.dumps(t.get('tags', [])), 'notes': t.get('notes', ''),
                'created_at': ts, 'updated_at': ts,
        ...
        for t in inc:
            task = {
                ...
                'completed_at': None, 'reminder_sent': 0, 'is_pinned': 0, 'progress': 0,
                'snooze_until': None
            }
            data['tasks'].insert(0, task)
            count += 1
